...
| ID | 状态 | 任务 | 验收标准 |
| --- | --- | --- | --- |
| SCH-06 | 已完成 | 抽取容量日历 | 日历构建是不依赖 React 和 Dexie 的纯函数模块。 |
| SCH-07 | 已完成 | 集中化约束判定 | 角色、团队、日期、每日任务和每周项目数限制共用同一入口。 |
| SCH-08 | 已完成 | 抽取纯排期引擎 | 不可变输入快照生成 `SchedulePlan`，计算期间不写数据库。 |
| SCH-09 | 已完成 | 按优先级组重排调度流程 | 每个优先级完成严格和跨团队放宽尝试后才进入下一级，优先级倒挂数为 0。 |

#### 第二阶段验收记录 - 2026-07-21

- 纯容量日历、集中约束和纯排期引擎已落地到 `extension/src/scheduling/`。
- `SchedulingContext` 仅负责读取数据、AI 技能评分、中断状态和排期结果落库；引擎计算期间无 Dexie 读写。
- 优先级组按「严格匹配 → 跨团队放宽收敛 → 下一优先级」执行。
- 跨 Pass 倒挂回归场景：高优先级 `cross-team` 项目获得全部 5 MD，低优先级 `all-in` 项目不得抢占，倒挂数为 0。
- 排期回归测试：6 项通过，0 项失败。
- 新增排期模块 ESLint 检查和 TypeScript/Vite 生产构建：通过。

### 第三阶段：资源利用率优化

| ID | 状态 | 任务 | 验收标准 |
| --- | --- | --- | --- |
| SCH-10 | 已完成 | 扫描全部空闲碎片 | 一名资源可以向同一项目贡献多个不连续的可行时间窗口。 |
| SCH-11 | 已完成 | 改进候选资源排序 | 稀缺度和碎片适配度优先于 AI 分数、Lead 匹配和最早开始日期。 |
| SCH-12 | 已完成 | 实现同优先级公平分配 | 容量不足时，同优先级项目之间的分配稳定且可解释。 |
| SCH-13 | 已完成 | 新增剩余容量收敛 | 不移动高优先级排期的前提下，使用剩余容量填补可行缺口。 |

#### 第三阶段验收记录 - 2026-07-21

- 空闲窗口扫描会返回资源在项目最早开始日之后的全部可行连续片段，同一资源可用多个不连续片段共同覆盖项目缺口。
- 同优先级先处理候选池更小的项目；候选资源依次按可替代项目数、碎片适配度、AI 分数、负责人匹配、最早窗口和资源 ID 排序。
- 容量不足时按当前同级项目数计算公平份额并循环收敛，固定项目 ID 作为确定性次级顺序。
- 所有优先级完成主排期后，按优先级从高到低执行剩余容量收敛，不删除或移动既有排期。
- 排期回归测试：10 项通过，0 项失败；第三阶段新增 4 项针对性测试。

### 第四阶段：交付安全与总体验收

| ID | 状态 | 任务 | 验收标准 |
| --- | --- | --- | --- |
| SCH-14 | 已完成 | 原子化写入排期 | 成功计划使用一个 Dexie 事务写入，取消和失败不留下部分计划。 |
| SCH-15 | 已完成 | 新增排期审计指标 | 报告包含优先级倒挂、过载、可匹配空闲 MD 和各优先级完成率。 |
| SCH-16 | 已完成 | 同步 UI 与文档 | 展示顺序、原因标签、PRD 和开发指南与引擎行为一致。 |
| SCH-17 | 已完成 | 执行性能和回归验收 | 代表性数据集结果确定、无正确性回退，并记录资源利用率变化。 |

#### 第四阶段验收记录 - 2026-07-21

- 排期记录删除、新计划新增和项目拒绝原因更新统一在一个 Dexie `rw` 事务内提交；进入事务前仍检查停止信号。
- 故障注入回归验证：项目原因更新失败时，已执行的排期删除与新增全部回滚，不留下部分计划。
- 项目结果页新增优先级倒挂、资源过载、可匹配空闲 MD、资源利用率和各优先级完成率；项目按优先级与稳定 ID 展示，未排原因支持中英文。
- 代表性数据集：20 名开发资源、60 个项目、30 个工作日；两次完整排期总耗时约 177ms，结果完全一致，资源利用率 50%，优先级倒挂 0，资源过载 0，可匹配空闲 MD 0。
- 排期回归测试：14 项通过，0 项失败；新增原子提交、失败回滚、审计指标、确定性和性能验收。
- 新增排期模块 ESLint 检查通过。Dashboard 与 React Context 的存量 `any`、Hooks 和 Fast Refresh 全量 ESLint 基线仍待独立治理。
