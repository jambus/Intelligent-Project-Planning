...
3. **节假日历反推**：系统导入修改后的天数时，严格调用本地预设的 `workingDaySet` 和节假日日历。系统能准确通过 `天数 / 实际工作日` 反推转换回原始的投入百分比（`allocationPercentage`）进行覆写落盘，杜绝人工计算误差。
...
* **多分配记录融合与替换**：引擎会自动清理目标周内的所有细碎分配片段，然后将其聚合成一条新的、完美贴合该周边界的单一 Allocation 记录，且新的占比百分比（Allocation Percentage）会根据该周实际工作日精准推算得出。
