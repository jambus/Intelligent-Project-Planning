...
系统预设以下 7 种标准研发角色，并自动与项目工时评估（MD）进行匹配：
*   **前端工程师 / 后端工程师 / APP工程师**：属于开发力量，主要负责 `devTotalMd`（开发工时）。
*   **全栈工程师**：**属于开发力量**，具有通用性，可灵活参与前端或后端的 `devTotalMd` 任务。全栈工程师只负责开发，不参与测试工作。
*   **开发组长**：属于开发力量，参与 `devTotalMd` 任务。在运维排期中被视为"保护对象"，优先级低于普通工程师。
*   **测试工程师**：专门负责 `testTotalMd`（测试工时）。
*   **测试组长**：属于测试力量，参与 `testTotalMd` 任务。在运维排期中被视为"保护对象"，优先级低于普通测试工程师。
...
#### 3.3.1 本地文件导入与项目管理
*   **优先级逻辑**：排期引擎使用标准化的优先级字段，严格按 `Must Win > P0 > Compliance > High/P1/高 > Medium/P2/中 > Low/P3/低 > 未识别` 从高到低排期。优先级值忽略大小写、首尾空格以及常见分隔符。
*   **同级顺序与日期语义**：同一优先级先处理可选资源更少的项目，并使用文件导入顺序（数据库自增 ID）作为稳定的次级顺序。项目 `startDate` 是最早开始硬约束，`endDate` 当前为期望日期，用户选择的排期区间是绝对物理边界。
*   **人员月度容量语义**：人员 `capacity` 表示当月可排完整工作日占有效工作日的比例，不是每日投入比例。例如当月有 20 个有效工作日时，50% 资源最多排 10 个整天，每个排期日计 1 MD。月度额度按 `floor(有效工作日 × capacity / 100)` 计算，请假日不计入有效工作日。
...
6.  **确定性贪心调度引擎 (Deterministic Greedy Engine)**：彻底移除了原先的多轮 AI 调度死循环。现在的本地排期引擎先遵守项目优先级，再综合资源稀缺度、时间碎片适配度和 AI 技能分执行确定性分配。
    ...
    *   **全碎片利用**：约束模块扫描资源的全部可行连续窗口；单个窗口不足时，同一资源可用多个不连续窗口共同填补一个项目的缺口。
    *   **稀缺与适配排序**：同级项目优先处理候选池更小者，资源候选依次比较可替代项目数、碎片是否足量及浪费量、AI 分数、负责人匹配、最早窗口和资源 ID。
    *   **同级公平份额**：每轮根据同级未完成项目和剩余可用容量计算公平份额，避免排在前面的同级项目一次耗尽共享资源。
    *   **剩余容量收敛**：主排期结束后按优先级从高到低重访缺口，仅使用尚未占用的可行窗口，不移动或删除既有高优先级分配。
    *   **原子化交付**：排期删除、新计划新增和项目拒绝原因更新在同一个 Dexie 读写事务中提交。用户停止发生在提交前时不写入，任一写入失败时整体回滚。
    *   **统一审计报告**：基于当前数据快照计算优先级倒挂、逐日及月度过载、可匹配空闲 MD、总利用率和各优先级完成率；未能匹配的剩余容量归类为无项目缺口、角色不匹配或团队/日期约束。
...
3. **确定性贪心调度 (Deterministic Greedy Allocation)**：彻底移除了初版的 AI 策略模式（专注/均衡/紧急）。在获取到 AI 的人员能力评分矩阵后，所有的分配行为交由 JS 引擎执行严谨的贪心匹配。
   * **负责人强制锁定 (Mandatory Leads)**：系统会自动识别项目定义的「技术负责人」与「质量负责人」。在给 AI 打分时，会直接无条件赋予满分，确保在贪心循环中，负责人被最优先分配到属于他们的项目。
   * **基于明细的技能匹配 (Detail-based Matching)**：AI 依旧会深度解析 `Details Product DEV/TEST MD` 字段中的具体产品明细，优先给具备相关业务领域知识或技术能力的研发人员打出高分。
   * **自定义打分指令 (Editable Scoring Prompt)**：在系统设置页面，用户可以自由修改 AI 的核心打分 Prompt。通过自定义 Prompt，团队可以轻松扩展独有的业务匹配准则（例如：禁止全栈接手核心前端项目等），且一键支持重置回系统默认逻辑。
...
7. **多项目并行约束 (Max Projects Per Week)**：每位成员每天只能执行一个任务（项目或运维），但在同一周内，系统放开了单一项目的独占锁定。现在，同一个资源每周最多允许并行服务 **3** 个不同的项目（分不同日期进行），兼顾了调度的灵活性与交付质量。周级排期结果中不会出现小数点——所有数值均为整数工作日。
8. **AI 返回结果合法性校验 (Schema Validation)**：在解析 AI 技能打分结果时，系统会强制执行 Schema 校验，过滤掉格式异常的返回，确保打分矩阵的安全导入和调度引擎的稳健。
...
*   **高内聚子页面拆分**：为了承载更大体量的数据，原单一大盘已被精细化拆解为四个独立职责的专属页面：
    *   **全局概览 (DashboardOverview)**：聚焦宏观管控。顶部常驻时间与排期控制台及 4 大核心 KPI 指标卡片。页面顶部固定展示**排期窗口容量提示横幅**，同时呈现"项目需求 MD"、"运维需求 MD"与"可用容量 MD"三维度对比，当总需求（项目+运维）超出可用容量时自动升级为橙色超载预警，并显示跨窗口项目数。下方直接展示完整的「Scrum 团队容量 (当前选定时段)」矩阵，让管理者一眼洞察各团队负荷健康度。
...
2. **智能"好资源"保护 (Leader Protection)**：在挑选运维人员时，系统会自动扫描当前所有待排期项目，将在项目中被提名为"技术负责 (Tech Lead)"或"质量负责 (Quality Lead)"的人员标记为保护对象。运维分配采用**三阶段优先级策略**：
   * **第一优先级**：同样具备该产品技能标签的"普通工程师"（非 Leader、非组长角色）优先承接运维；
   * **第二优先级**：仅当普通工程师无法覆盖时，才将项目负责人（Tech Lead / QA Responsible）纳入候选池；
   * **第三优先级（兆底）**：仅当前两类资源均无法满足当月需求时，“开发组长”或“测试组长”才会被分配运维。
   此机制最大限度地将组长的时间保留给项目排期，确保团队管理者不会因运维任务而影响项目交付。
...
8. **登录态智能检测 (Auth State Detection)**：底层网络请求引擎能够主动探测 `401 Unauthorized` 和 `403 Forbidden` 等未授权状态，并能识别出被 Jira 重定向到登录页面的 HTML 响应。此外，系统还具备**数据级空响应检测**：当同步完成后所有目标项目的工时、状态、子任务数均为零时，判定为登录态失效（区别于单个项目无数据的正常情况），立即阻断写入并弹窗强制提醒用户在新标签页中完成 Jira 登录，避免将无效的全零数据覆盖数据库中的历史同步结果。
...
### 3.3.17 高级排期矩阵编辑与行锁定
- **人员分组排序**：排期明细矩阵切换为“按人员分组”时，先按研发资源姓名排序，再按项目名称和项目 ID 稳定排序，确保同一成员的全部排期连续展示。
