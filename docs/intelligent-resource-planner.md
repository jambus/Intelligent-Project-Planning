...
*   **独立子视图拆分与聚合**：原全局大盘被精细化拆分为 `全局概览`、`已排项目`、`已排期任务详情` 以及 `待跟进事项` 四大核心子菜单视图，支持通过左侧层级菜单直达对应面板。
*   **排期状态细化与归因推断**：废弃简单的二元展示，将项目按排班满足率细分为“已排满”、“部分排上”、“排不上”三个列表。同时新增智能启发式归因，系统会在“排不上项目”后标注推测原因，极大降低了人工对齐资源的时间成本。
*   **Scrum 团队容量大盘**：在概览中引入 Scrum 团队维度的实时容量水位条，自动按所选时间范围累加各个团队的剩余容量、开发/测试闲置人天及整体饱和度，助力宏观层面的负载均衡调配。
