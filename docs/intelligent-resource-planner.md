...
5.  **持久化后台调度与手动中断 (Persistent Background Scheduling & Manual Stop)**：
    *   **全局上下文 (Scheduling Context)**：排期引擎已移至根级 Context 维护。用户在启动排期后，可以自由切换到人员管理、技能管理或系统设置页面，排期任务将在后台持续运行而不会中断。
    *   **实时进度同步**：无论何时返回大盘页面，系统都会自动同步当前的后台排期进度、状态文案与进度条。
    *   **毫秒级响应中断 (Aggressive Interruption)**：新增「停止排期」功能。系统在每个异步操作点埋入了激进的哨兵检查（checkStop），点击停止后会立即抛出异常并切断所有后续 AI 请求和数据库事务，确保资源的即时释放。
