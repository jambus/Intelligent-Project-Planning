...
3. **项目级排期策略与配置 (Project-level Strategies)**：新增“专注模式(100%投入)”、“均衡模式(50%分时并行)”与“紧急模式(超负荷推进)”等策略，并在项目管理中可逐项目定制。排期策略直接下沉到项目级别。在 CSV 导入时，系统会自动判断：如果项目为高优先级且总工时 > 10 人天，则默认采用“均衡模式”；低优先级或总工时 <= 10 人天的小项目则默认采用“专注模式”。用户也可在项目列表中单独覆盖修改。
...
*   **区块折叠交互 (Collapsible Sections)**：全局大盘中的 5 个核心区块（已排项目、已排期任务详情、待跟进项目、待补充任务、待评估项目）均支持点击表头折叠/展开，大幅降低页面复杂度，减少无意义的滚动，提升核心数据的专注度。
