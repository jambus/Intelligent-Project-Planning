...
*   **Scrum 团队管理 (Scrum Teams Management)**：
    *   独立的 Scrum 团队管理模块，支持按 Scrum 组织研发与测试资源，支持跨团队分配人员与安全的级联解除策略。人员的 CSV 导入与导出操作全面集成 `Scrum Team` 的自动识别与新建。
...
6. **硬性“单周单项目”独占分配 (Single Project Per Week)**：在生成候选人日历容量时引入周级别的排他性锁定。一旦某员工在一周内被分配了一个非运维项目，其在该自然周内将被判定为零产能，不再承接其他新项目。AI Prompt 中同步增加了严格指令约束，确保人员精力聚焦。
7. **AI 返回结果合法性校验 (Schema Validation)**：在解析 AI 建议时，系统会强制执行 Schema 校验，过滤掉 `projectId`/`resourceId` 缺失、分配人天小于 1 或百分比异常的非法条目，确保调度逻辑的鲁棒性。
...
*   **周级别动态计算公式**：基于 ISO 8601 自然周边界计算。`周投入人天 = 该周内项目重叠的工作日天数 * 投入占比 %`。

#### 3.3.5 数据展示与大盘增强 (Dashboard Enhancements)
*   **立体双层表头**：全局大盘支持“按月统领，按自然周细分”的双层多级表头视图，极大地提升了季度和月度内的精细化排期能见度。
*   **独立子视图拆分与聚合**：原全局大盘被精细化拆分为 `已排项目`、`已排期任务详情` 以及 `待跟进事项` 三大核心子菜单视图，支持通过左侧层级菜单直达对应面板。
*   **全局常驻控制台**：在切换不同的大盘子视图时，顶部的年份/月份选取、一键智能排期等核心操作台以及 4 大数字概览卡片始终置顶可见，保障跨视图操作连续性。
*   **碎片排期横向聚合展示**：系统渲染时会按 `(人员, 项目)` 为组合键进行强制折叠。即便同一人员在某项目上因为多种原因断断续续被拆出多段排期，在报表上也会平铺汇总为唯一行，起止日期合并显示，彻底解决了排期结果垂直堆叠导致表格臃肿的痛点。
*   **双向统计视图**：在 `已排期任务详情` 中，支持按「人员维度」查看负荷分布，或按「项目维度」查看资源组成。
*   **实时异常追踪**：通过 `待跟进事项` 视图，动态展示当前尚未满载的人员以及开发/测试缺口的残余人天。
